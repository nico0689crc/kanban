...
class ResponseParser {
  constructor({
    model, documents, request, totalDocuments,
  }) {
  ...
  parseDataCollection() {
    const getOriginalUrlFiltered = () => {
      ...
      const urlQueryFiltered = urlQuery
        .split('&')
        .filter((query) => !/page/.test(query))
        .join('&');

      const result = urlQueryFiltered.length > 0
        ? `${process.env.FRONTEND_HOST_DOMAIN}${urlBase}?${urlQueryFiltered}&`
        : `${process.env.FRONTEND_HOST_DOMAIN}${urlBase}?${urlQueryFiltered}`;
    ...
    const getLinks = () => {
      if (!this.pageSize || !this.pageNumber) {
        return {
          self: `${process.env.FRONTEND_HOST_DOMAIN}${this.request.originalUrl}`,
        };
      }
    ...
    this.documents.forEach(async (document) => {
      this.response.data.push({
        ...
        links: {
          self: `${process.env.BACKEND_HOST_DOMAIN}/api/${this.entityName}/${document.uuid}`,
        },
      });
    });
  }
  ...
  parseDataIndividual() {
    this.response = {
      links: {
        self: `${process.env.BACKEND_HOST_DOMAIN}/api/${this.entityName}/${this.documents.uuid}`,
      },
      data: {
        type: this.entityName,
