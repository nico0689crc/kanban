...
const registerUser = (req, res, next) => {
  ErrorHandler(async () => {
    ...
    const user = await User.sequelize.transaction(async () => {
      ...
      activationLink = `${process.env.FRONTEND_HOST_DOMAIN}/auth/verify?email=${user.dataValues.email}`;
...
const registerUserDemo = (req, res, next) => {
  ErrorHandler(async () => {
    ...
    const passwordHashed = await bcryptjs.hash(process.env.PASSWORD_DEMO, 12);
...
const requestResetPassword = async (req, res, next) => {
  ErrorHandler(async () => {
    ...
    const passwordResetLink = `${process.env.FRONTEND_HOST_DOMAIN}/auth/reset-password?uuid=${user.dataValues.uuid}&token=${passwordResetToken}`;
